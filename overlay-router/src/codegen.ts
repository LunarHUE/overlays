...
function generateTypeDefinitions() {
  ...
  lines.push("  }");
  lines.push("");
  lines.push("  interface OverlayProps<T extends Record<string, any> = any> {");
  lines.push("    data: T;");
...
function generateRouter() {
  ...
  lines.push(`import { useOverlay } from "./hooks";`);
  lines.push(``);
  lines.push(`export function OverlayRouter() {`);
  lines.push(`  const { currentOverlay } = useOverlay();`);
  lines.push(``);
  lines.push(`  if (!currentOverlay) return null;`);
  lines.push(``);
  lines.push(`  return <ServeOverlay overlayName={currentOverlay.name} data={currentOverlay.data} />;`);
  lines.push(`}`);
  lines.push(``);
...
function generateHooks() {
  ...
  lines.push(``);
  lines.push(`const STORAGE_KEY = "overlay_router_state";`);
  lines.push(`const EVENT_KEY = "overlay_router_change";`);
  lines.push(``);
  lines.push(`/**`);
  lines.push(` * Hook to manage overlay state through Session Storage`);
  lines.push(` */`);
  lines.push(`export function useOverlay() {`);
  lines.push(`  const [currentOverlay, setCurrentOverlay] = useState<{name: string, data: any} | null>(null);`);
  lines.push(``);
  lines.push(`  useEffect(() => {`);
  lines.push(`    const checkOverlay = () => {`);
  lines.push(`      try {`);
  lines.push(`        const item = sessionStorage.getItem(STORAGE_KEY);`);
  lines.push(`        if (item) {`);
  lines.push(`          setCurrentOverlay(JSON.parse(item));`);
  lines.push(`        } else {`);
  lines.push(`          setCurrentOverlay(null);`);
  lines.push(`        }`);
  lines.push(`      } catch (e) {`);
  lines.push(`        console.error("Failed to parse overlay state:", e);`);
  lines.push(`        setCurrentOverlay(null);`);
  lines.push(`      }`);
  lines.push(`    };`);
  lines.push(``);
  lines.push(`    checkOverlay();`);
  lines.push(`    window.addEventListener(EVENT_KEY, checkOverlay);`);
  lines.push(`    return () => window.removeEventListener(EVENT_KEY, checkOverlay);`);
  lines.push(`  }, []);`);
  lines.push(``);
  ...
  lines.push(`    ) => {`);
  lines.push(`      const state = { name, data };`);
  lines.push(`      sessionStorage.setItem(STORAGE_KEY, JSON.stringify(state));`);
  lines.push(`      window.dispatchEvent(new Event(EVENT_KEY));`);
  lines.push(`    },`);
  lines.push(`    []`);
  lines.push(`  );`);
  lines.push(``);
  lines.push(`  const closeOverlay = useCallback(() => {`);
  lines.push(`    sessionStorage.removeItem(STORAGE_KEY);`);
  lines.push(`    window.dispatchEvent(new Event(EVENT_KEY));`);
  lines.push(`  }, []);`);
  lines.push(``);
  ...
  lines.push(`      const item = sessionStorage.getItem(STORAGE_KEY);`);
  lines.push(`      if (item) {`);
  lines.push(`        const current = JSON.parse(item);`);
  lines.push(`        const newState = { ...current, data };`);
  lines.push(`        sessionStorage.setItem(STORAGE_KEY, JSON.stringify(newState));`);
  lines.push(`        window.dispatchEvent(new Event(EVENT_KEY));`);
  lines.push(`      }`);
  lines.push(`    },`);
  lines.push(`    []`);
